...
StructuredChannel.prototype = {
  ...
  _handleMessage: function(event) {
    ...
    handlerResult.then(result => {
      this.port.postMessage({ type: REPLY_TYPE, error: false, result, id });
    }, err => {
      this.port.postMessage({ type: REPLY_TYPE, error: err, id });
    });
  },
};
...
StructuredChannel.connectTo = function(target, targetOrigin, global) {
  if (targetOrigin instanceof Object &&
      targetOrigin.MessageChannel instanceof Function)
  {
